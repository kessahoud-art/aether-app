...
export async function echoSouffle(userId, souffleId, comment = '') {
  const { data: exist } = await supabase
    .from('echos')
    .select('id')
    .eq('entity_id', userId)
  ...
  await supabase.from('echos')
    .insert({ entity_id: userId, souffle_id: souffleId, comment })
  await supabase.rpc('increment_karma', { uid: userId, delta: 3 })
  return { action: 'added' }
}
